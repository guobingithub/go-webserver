...
import (
	...
	"apusic/go-webserver/src/utils/k8s/pod-log/k8s"
...
func GetKubeLogs(c *gin.Context) {
	// /api/v1/namespaces/k8s-system/pods/traefik-7cb4cb6bf5-p779x/logs?tailLines=500&timestamps=true&previous=false&container=traefik'
	...
	klog.V(2).InfoS("get k8s logs request params", "namespace", namespace, "pod", podName,
	...
	if err := k8s.Client.Pod.LogStream(podName, namespace, &opts, kubeLogger); err != nil {
